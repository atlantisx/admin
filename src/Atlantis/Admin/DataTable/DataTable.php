<?php
namespace Atlantis\Admin\DataTable;
...
class DataTable {
	 ...
	 *
	 * @var \Atlantis\Admin\DataTable\Columns\Factory
	 */
	 ...
	 * The validator instance
	 *
	 * @var \Atlantis\Admin\Fields\Factory
	 */
	 ...
	 * @param \Atlantis\Admin\DataTable\Columns\Factory	$columnFactory
	 * @param \Atlantis\Admin\Fields\Factory				$fieldFactory
	 */
	...
	public function setRowsPerPage(\Illuminate\Session\Store $session, $globalPerPage, $override = null)
	{
		if ($override)
		{
			...
			$session->put('admin_' . $this->config->getOption('name') . '_rows_per_page', $perPage);
		}

		$perPage = $session->get('admin_' . $this->config->getOption('name') . '_rows_per_page');
